...
def test_notebook_has_job_configuration_and_no_saved_execution_output() -> None:
    notebook = json.loads(NOTEBOOK_PATH.read_text())
    code = _notebook_code()

    assert 'ensure_widget("lakebase_endpoint", os.getenv("LAKEBASE_ENDPOINT",' in code
    assert 'ensure_widget("pg_host", os.getenv("PGHOST",' in code
    assert 'ensure_widget("pg_user", os.getenv("PGUSER",' in code
    ...
    for name in ("lakebase_endpoint", "pg_host", "pg_user"):
        assert widgets[name]["currentValue"].strip()
        assert widgets[name]["typedWidgetInfo"]["defaultValue"].strip()
        assert widgets[name]["widgetInfo"]["defaultValue"].strip()
